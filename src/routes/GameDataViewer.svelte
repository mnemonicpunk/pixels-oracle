<script>
  ...
  const config = {
    ...
    sprite: (value) =>
      `<img src=${value.image} style="width: ${value.width}px, height: ${value.height}px, ">`,
  ...
  const defaultRender = (key, value) => {
    if (typeof value === "object" && value !== null) {
      return Object.keys(value)
        .map((subKey) => renderKey(subKey, value[subKey]))
        .join("\n");
    } else {
      return `${key}: ${value}`;
    }
    ...
    {:else}
      ...
      {#if key == "craftable"}
        {#if data[key].requiredSkill}
          <div class="viewer_card">
            Requires <b
              >{data[key].requiredSkill}{data[key].requiredLevel
                ? " " + data[key].requiredLevel + " "
                : ""}</b
            >
          </div>
        {/if}
        <div class="viewer_card">
          Crafting ({data[key].minutesRequired} minute{data[key]
            .minutesRequired > 1
            ? "s"
            : ""}, {data[key].result?.energyUsage?.value} energy):<br />
          <ul>
        ...
        <div class="viewer_card">
          {#each Object.values(data[key]) as item}
            <div class="viewer_card_list_item">
              ...
              {#if item.buyPrice}
                <div
                  class="viewer_card_tag{item.buyStatus == 'lock' ||
                  item.buyStatus == 'hide' ||
                  item.buyStatus === null
                    ? ' locked_tag'
                    : ''}"
                >
                  Buy: {item.buyPrice || "n/a"}
                  <img
                    style="width: 12px; height: 12px"
                    src={item.currency == "cur_coins"
                      ? "//d31ss916pli4td.cloudfront.net/uploadedAssets/currency/cur_coins/abf1e7f3-994d-46d0-a7d1-a0bdbef2c4b1.png"
                      : "//d31ss916pli4td.cloudfront.net/uploadedAssets/currency/pixels_pixel_currency_new_2.png"}
                    alt="currency"
                  />
                </div>
              {/if}
